...
func printHelp(w io.Writer, prog string) {
	...
	fmt.Fprintf(w, "  s                     toggle selection on current item (and its subtree)\n")
	fmt.Fprintf(w, "  Esc                   clear the whole selection, tree-wide\n")
	...
	fmt.Fprintf(w, "                        (with a/b/c/d, acts on the whole selection if one exists)\n")
	...
	fmt.Fprintf(w, "  R                     three-way only: mark current item's subtree as resolved\n")
	fmt.Fprintf(w, "                        (a/b/c/d/m/M/n/R are disabled in --read-only mode)\n")
