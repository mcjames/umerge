...
class Model:
    ...
    #
    #
    def __init__(self, fileops, left, right):
        ...
        self.top = Match2.Match(left, right, None)
    ...
    def request_operation(self, operation, item=None):
        ...
        # do nothing.
        if (operation != "refresh" and item is not None
                and item.has_uncompared_descendants()):
            # print("has uncompared descendants. Doing nothing...")
            return False

        if operation == "refresh":
            if item is not None:
                item.set_state_of_tree(Match2.UNCOMPARED)  # This looks wrong
    ...
    def __enumerate_aux(self):
        with self.lock():
            ...
            self.top.enumerate()
            self.top.set_state_of_tree(Match2.UNCOMPARED)
            self.top.lm_num_diffs = None
            self.top.mr_num_diffs = None
            self.top.lr_num_diffs = None
            if self.top.resolution_status != ' ':
                self.top.set_resolution_status_of_tree(
                    self.top.resolution_status)
    ...
    def compare_match_item(self, item):
        if self.stop_comparison:
            return
        if item.state == Match2.UNCOMPARED:
...
#         dest = self.operation_copy_dest
        ...
        if result is None:
            ...
            item.set_state_of_tree(Match2.ERROR)
            self.modelstate = STATE_NORMAL
            self.operation_thread = None
    ...
    def __delete_item(self, item):
        ...
        if (exit_status_left is not None
            ...
            item.set_state_of_tree(Match2.DELETED)
