...
class View:
    ...
    def __render_roots_line( self ):
        ...
        middle = self.model.top.middle
        if len(middle) > self.middle_item_width:
            middle = middle[:self.middle_item_width]
        elif len(middle) < self.middle_item_width:
            middle += ' ' * (self.middle_item_width - len( middle ))
        ...
        self.canvas.draw_text( 0, col, middle, TOP_LINE )
        col += len(middle)
    ...
    def __recalculate_sizes( self ):
        ...
        self.left_item_width = (self.cols - 2) // 3
        self.middle_item_width = self.left_item_width
        self.right_item_width = self.left_item_width
        if (self.cols - 2) % 3 == 1:
            self.left_item_width += 1
        elif (self.cols - 2) % 3 == 2:
            self.left_item_width += 1
            self.middle_item_width += 1
        assert( self.left_item_width + self.middle_item_width
                + self.right_item_width + 2 == self.cols )
        #print( 'left_item_width =', self.left_item_width )
        #print( 'middle_item_width =', self.left_item_width )
...
#         return result
    ...
    def __render_partial_path_item( self,
        ...
        if lmr == LEFT:
            name = item.left
            num_diffs = item.lm_num_diffs
            desired_size = self.left_item_width
            if True: #self.model.merge_done:
                desired_size -= 1
        elif lmr == MIDDLE:
            name = item.middle
            num_diffs = item.mr_num_diffs
            desired_size = self.middle_item_width
        else:
            name = item.right
            num_diffs = item.lr_num_diffs
...
#            self.__render_line( row, col, result, color )
    ...
    def __fixed_compute_colors( self, is_cursor, item ):
        ...
        new_colors[4] = colors[2]
        new_colors[5] = SELECTED
    ...
    def __compute_colors( self, item ):
        ...
        if item.FilesAreUncompared():
            ...
            return [UNCOMPARED, UNCOMPARED, UNCOMPARED]

        if item.hidden:
            # fixme: I could display green too
            return [NORMAL_HIDDEN, NORMAL_HIDDEN, NORMAL_HIDDEN]
    ...
    def __init_color_pairs( self ):
        self.__init_a_color_pair( NORMAL_DIR_ARROW,    'normal_dir_arrow_fg',    'normal_dir_arrow_bg' )
        self.__init_a_color_pair( NORMAL_FILENAME,     'normal_filename_fg',     'normal_filename_bg' )
        self.__init_a_color_pair( NORMAL_COUNT,        'normal_count_fg',        'normal_count_bg' )
        self.__init_a_color_pair( NORMAL_VERTICAL_SEP, 'normal_vertical_sep_fg', 'normal_vertical_sep_bg' )
        self.__init_a_color_pair( NORMAL_HIDDEN,       'normal_hidden_fg',       'normal_hidden_bg' )

        self.__init_a_color_pair( NORMAL_DIR_ARROW_H,    'normal_dir_arrow_h_fg',    'normal_dir_arrow_h_bg' )
        self.__init_a_color_pair( NORMAL_FILENAME_H,     'normal_filename_h_fg',     'normal_filename_h_bg' )
        self.__init_a_color_pair( NORMAL_COUNT_H,        'normal_count_h_fg',        'normal_count_h_bg' )
        self.__init_a_color_pair( NORMAL_VERTICAL_SEP_H, 'normal_vertical_sep_h_fg', 'normal_vertical_sep_h_bg' )
        self.__init_a_color_pair( NORMAL_HIDDEN_H,       'normal_hidden_h_fg',       'normal_hidden_h_bg' )

        self.__init_a_color_pair( CHANGED_DIR_ARROW,    'changed_dir_arrow_fg',    'changed_dir_arrow_bg' )
        self.__init_a_color_pair( CHANGED_FILENAME,     'changed_filename_fg',     'changed_filename_bg' )
        self.__init_a_color_pair( CHANGED_COUNT,        'changed_count_fg',        'changed_count_bg' )
        self.__init_a_color_pair( CHANGED_VERTICAL_SEP, 'changed_vertical_sep_fg', 'changed_vertical_sep_bg' )
        self.__init_a_color_pair( CHANGED_HIDDEN,       'changed_hidden_fg',       'changed_hidden_bg' )

        self.__init_a_color_pair( CHANGED_DIR_ARROW_H,    'changed_dir_arrow_h_fg',    'changed_dir_arrow_h_bg' )
        self.__init_a_color_pair( CHANGED_FILENAME_H,     'changed_filename_h_fg',     'changed_filename_h_bg' )
        self.__init_a_color_pair( CHANGED_COUNT_H,        'changed_count_h_fg',        'changed_count_h_bg' )
        self.__init_a_color_pair( CHANGED_VERTICAL_SEP_H, 'changed_vertical_sep_h_fg', 'changed_vertical_sep_h_bg' )
        self.__init_a_color_pair( CHANGED_HIDDEN_H,       'changed_hidden_h_fg',       'changed_hidden_h_bg' )

        self.__init_a_color_pair( INSERTED_DIR_ARROW,    'inserted_dir_arrow_fg',    'inserted_dir_arrow_bg' )
        self.__init_a_color_pair( INSERTED_FILENAME,     'inserted_filename_fg',     'inserted_filename_bg' )
        self.__init_a_color_pair( INSERTED_COUNT,        'inserted_count_fg',        'inserted_count_bg' )
        self.__init_a_color_pair( INSERTED_VERTICAL_SEP, 'inserted_vertical_sep_fg', 'inserted_vertical_sep_bg' )
        self.__init_a_color_pair( INSERTED_HIDDEN,       'inserted_hidden_fg',       'inserted_hidden_bg' )

        self.__init_a_color_pair( INSERTED_DIR_ARROW_H,    'inserted_dir_arrow_h_fg',    'inserted_dir_arrow_h_bg' )
        self.__init_a_color_pair( INSERTED_FILENAME_H,     'inserted_filename_h_fg',     'inserted_filename_h_bg' )
        self.__init_a_color_pair( INSERTED_COUNT_H,        'inserted_count_h_fg',        'inserted_count_h_bg' )
        self.__init_a_color_pair( INSERTED_VERTICAL_SEP_H, 'inserted_vertical_sep_h_fg', 'inserted_vertical_sep_h_bg' )
        self.__init_a_color_pair( INSERTED_HIDDEN_H,       'inserted_hidden_h_fg',       'inserted_hidden_h_bg' )

        self.__init_a_color_pair( REMOVED_DIR_ARROW,    'removed_dir_arrow_fg',    'removed_dir_arrow_bg' )
        self.__init_a_color_pair( REMOVED_FILENAME,     'removed_filename_fg',     'removed_filename_bg' )
        self.__init_a_color_pair( REMOVED_COUNT,        'removed_count_fg',        'removed_count_bg' )
        self.__init_a_color_pair( REMOVED_VERTICAL_SEP, 'removed_vertical_sep_fg', 'removed_vertical_sep_bg' )
        self.__init_a_color_pair( REMOVED_HIDDEN,       'removed_hidden_fg',       'removed_hidden_bg' )

        self.__init_a_color_pair( REMOVED_DIR_ARROW_H,    'removed_dir_arrow_h_fg',    'removed_dir_arrow_h_bg' )
        self.__init_a_color_pair( REMOVED_FILENAME_H,     'removed_filename_h_fg',     'removed_filename_h_bg' )
        self.__init_a_color_pair( REMOVED_COUNT_H,        'removed_count_h_fg',        'removed_count_h_bg' )
        self.__init_a_color_pair( REMOVED_VERTICAL_SEP_H, 'removed_vertical_sep_h_fg', 'removed_vertical_sep_h_bg' )
        self.__init_a_color_pair( REMOVED_HIDDEN_H,       'removed_hidden_h_fg',       'removed_hidden_h_bg' )

        self.__init_a_color_pair( ABSENT,       'absent_fg',       'absent_bg' )
        self.__init_a_color_pair( ABSENT_H,     'absent_h_fg',     'absent_h_bg' )
        self.__init_a_color_pair( UNCOMPARED,   'uncompared_fg',   'uncompared_bg' )
        self.__init_a_color_pair( UNCOMPARED_H, 'uncompared_h_fg', 'uncompared_h_bg' )
        self.__init_a_color_pair( ERROR,        'error_fg',        'error_bg' )
        self.__init_a_color_pair( ERROR_H,      'error_h_fg',      'error_h_bg' )
        self.__init_a_color_pair( SELECTED,     'selected_fg',     'selected_bg' )
        self.__init_a_color_pair( SELECTED_H,   'selected_h_fg',   'selected_h_bg' )

        self.__init_a_color_pair( MARKER_OK,       'marker_ok_fg',       'marker_ok_bg' )
        self.__init_a_color_pair( MARKER_MERGED,   'marker_merged_fg',   'marker_merged_bg' )
        self.__init_a_color_pair( MARKER_RESOLVED, 'marker_resolved_fg', 'marker_resolved_bg' )
        self.__init_a_color_pair( MARKER_CONFLICT, 'marker_conflict_fg', 'marker_conflict_bg' )

        self.__init_a_color_pair( TOP_LINE,         'top_line_fg',         'top_line_bg' )
        self.__init_a_color_pair( TOP_VERTICAL_SEP, 'top_vertical_sep_fg', 'top_vertical_sep_bg' )

        self.__init_a_color_pair( STATUS_1,         'status_1_fg',         'status_1_bg' )
        self.__init_a_color_pair( STATUS_2,         'status_2_fg',         'status_2_bg' )
        self.__init_a_color_pair( STATUS_3,         'status_3_fg',         'status_3_bg' )
        self.__init_a_color_pair( STATUS_4,         'status_4_fg',         'status_4_bg' )

        self.__init_a_color_pair( PROMPT_1,         'prompt_1_fg',         'prompt_1_bg' )
        self.__init_a_color_pair( PROMPT_2,         'prompt_2_fg',         'prompt_2_bg' )
        self.__init_a_color_pair( PROMPT_3,         'prompt_3_fg',         'prompt_3_bg' )
        self.__init_a_color_pair( PROMPT_4,         'prompt_4_fg',         'prompt_4_bg' )
